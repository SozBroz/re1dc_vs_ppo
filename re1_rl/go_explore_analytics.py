...
class SemanticBucket:
    ...
    @property
    def quality_best(self) -> tuple[int, int, int, int, int] | None:
        if not self.cells:
            return None
...
def _as_quality(raw: Any) -> tuple[int, int, int, int, int]:
    if not isinstance(raw, (list, tuple)) or len(raw) < 5:
        return (0, 0, 0, 0, 0)
    return tuple(int(x) for x in raw[:5])  # type: ignore[return-value]

...
def eligible_reset_cells(cells: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Cells the random reset sampler may use (cp00–cp115; never the terminal cp116)."""
    return [
        row
        for row in cells
...
def _choose_reset_candidate(
    ...
    *,
    ...
    """Uniform over eligible cp00–cp115 cells (no fresh start; no latest bias)."""
    eligible = eligible_reset_cells(cells)
    if not eligible:
        raise ValueError(
...
def sample_one_leg_options(
    ...
    *,
    ...
    loadable cell in cp00–cp115. Completing the hunted cell ends the episode
    ...
    if not cells:
        return _fresh_start_options(stage)
    # Equal chance: true fresh start + each loadable cp00–cp115 cell.

"""cp41 (storeroom enter) requires clearing the East Stairway zombie in 10B."""
...
def test_route_cp41_enter_is_room_only_kills_are_capture() -> None:
    cp = next(row for row in _ROUTE if row["checkpoint_id"] == "storeroom_enter_118")
    assert cp["success_condition"] == {"type": "room_enter", "room_id": "118"}


def test_storeroom_planner_pays_enter_without_10b_kill() -> None:
    planner = _planner("storeroom_enter_118")
...
def test_capture_rejects_storeroom_without_10b_kill(
    ...
    planner = _planner("storeroom_enter_118")
    planner._index = int(_ROUTE_INDEX["storeroom_enter_118"]) + 1
    progress = ProgressTracker()
    progress.leg_kills_by_room["10B"] = 0
    env = SimpleNamespace(
        ...
        _progress=progress,
        _step_count=400,
        _read_state=lambda track_items=False: _state("118"),
    )
    monkeypatch.setattr(
    ...
    progress.leg_kills_by_room["10B"] = 1
    ...
    assert proposal["checkpoint_id"] == "storeroom_enter_118"
    assert proposal["room_id"] == "118"

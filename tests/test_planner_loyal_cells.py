...
def test_training_starts_are_pl06_and_every_later_slot(
    ...
    _write_cell(tmp_path, TRAINING_START_INDEX, meta={"training_start": True})
    ...
    assert [int(row["checkpoint_index"]) for row in starts] == [
        TRAINING_START_INDEX,
        10,
        11,
        18,
...
def test_pl00_fresh_start_flag_counts_below_tip(
    ...
    _write_cell(tmp_path, TRAINING_START_INDEX, meta={})
    starts = iter_training_start_cells(tmp_path)
    assert [int(row["checkpoint_index"]) for row in starts] == [0, TRAINING_START_INDEX]
...
def test_pl00_opening_chunk_stays_a_start_under_live_chunk(
    ...
    _write_cell(tmp_path, TRAINING_START_INDEX, meta={"chunk_id": "cp05_shield_key"})
    starts = iter_training_start_cells(tmp_path)
    assert [int(row["checkpoint_index"]) for row in starts] == [0, TRAINING_START_INDEX]
...
def test_pl00_recomp_meta_is_not_a_bizhawk_start(
    ...
    (
        ...
        / "planner_loyal"
        / "cells"
        / cell_dir_name(TRAINING_START_INDEX)
        / "cell.pst"
    ).write_bytes(b"PST")
...
def test_exhausted_last_step_cell_is_not_a_start(
    ...
    _write_cell(tmp_path, TRAINING_START_INDEX, meta={"planner_step_index": None})
    _write_cell(
        tmp_path,
        TRAINING_START_INDEX + 1,
        meta={"planner_step_index": 0, "chunk_id": "short"},
    )
    _write_cell(
        tmp_path,
        TRAINING_START_INDEX + 2,
        meta={
            "planner_step_index": 1,
            "chunk_final": True,
            "chunk_id": "short",
...
def test_cells_from_another_chunk_only_qualify_at_the_tip(
    ...
    # training_start true; the lockpick final mint is the live tip with seek 0.
    for idx in range(1, TRAINING_START_INDEX):
        _write_cell(
            tmp_path,
            idx,
            meta={
    ...
    _write_cell(
        ...
        TRAINING_START_INDEX,
        meta={
            "planner_step_index": TRAINING_START_INDEX - 1,
            "chunk_id": "opening_to_lockpick",
            "chunk_final": True,
            "training_start": True,
        },
    ...
    _write_cell(
        tmp_path,
        TRAINING_START_INDEX + 1,
        meta={"planner_step_index": 0, "chunk_id": "cp05_shield_key"},
    )
    starts = iter_training_start_cells(tmp_path)
    assert [int(row["checkpoint_index"]) for row in starts] == [
        TRAINING_START_INDEX,
...
def _seed_starts(tmp_path: Path) -> None:
    for idx in (TRAINING_START_INDEX, 10, 11, 13):
...
def test_reset_pin_set_and_unminted_fallback(
    ...
    _clear_pin_env(monkeypatch)
    _seed_starts(tmp_path)
    _write_pin(tmp_path, f"RE1_PLANNER_RESET_PIN_SET={TRAINING_START_INDEX},11\n")
    assert [int(row["checkpoint_index"]) for row in iter_training_start_cells(tmp_path)] == [
        TRAINING_START_INDEX,
        11,
    ]
...
def test_reset_pin_set_weight_mixes_named_and_rest(
    ...
    assert counts[TRAINING_START_INDEX] > 20 and counts[10] > 20 and counts[13] > 20
...
def test_reset_pin_weights_rest_and_exclusive(
    ...
    assert counts[TRAINING_START_INDEX] > 20 and counts[10] > 20 and counts[13] > 20
    _write_pin(
        tmp_path, f"RE1_PLANNER_RESET_PIN_WEIGHTS={TRAINING_START_INDEX}:1,11:1\n"
    )

...
def test_door_paired_tea_room_scene_key_is_kenneth() -> None:
    """Door+Kenneth same skip: ``104:*:sN`` is still the Kenneth flag."""
    progress = ProgressTracker()
    progress.seed_spawn_room("105")
    prev = make_state(room="105", cam_id=2, hp=96)
    cur = make_state(
...
def test_illegal_main_hall_gate_irreversibly_disables_positive_rewards() -> None:
    ...
    assert bounce_bd[ILLEGAL_MAIN_HALL_FAILURE_REASON] == MAIN_HALL_BEFORE_KENNETH_PENALTY
...
def test_real_death_owns_penalty_not_kenneth_gate() -> None:
    ...
    assert MAIN_HALL_BEFORE_KENNETH_PENALTY == -4.0

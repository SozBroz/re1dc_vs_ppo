...
def test_default_mix_equal_fresh_and_each_loadable_cell(tmp_path: Path) -> None:
    """Fresh start is its own slot, equal with each cp00–cp107 cell — not cp00."""
    manifest = {
        "schema_version": 1,
        "route_id": "test",
        "cells": [_write_cell(tmp_path, i) for i in (0, 18, 107)],
    }
    (tmp_path / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    stage = {
        "route_id": "test",
        "cells_manifest": "manifest.json",
        "route_steps": list(range(1, 110)),
    }
    counts: dict[str, int] = {"fresh": 0, "cp00": 0, "cp18": 0, "cp107": 0}
...
def test_cp107_playthrough_is_the_yawn_fight_only(tmp_path: Path) -> None:
    manifest = {
        "schema_version": 1,
        "route_id": "test",
        "cells": [_write_cell(tmp_path, 107)],
    }
    (tmp_path / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    stage = {
        "route_id": "test",
    ...
    for seed in range(200):
        opts = sample_one_leg_options(tmp_path, stage, rng=random.Random(seed))
        if opts.get("route_start_index") == 108:
            monkey_hits += 1
            assert opts["leg_span"] == 1
...
def test_terminal_cp108_is_never_loadable(tmp_path: Path) -> None:
    """cp108 has no next hunt target — no agent may reset into it."""
    import shutil
    ...
    for idx in (107, 108):
        ...
        row["next_checkpoint_id"] = "" if idx == 108 else "yawn_moon_210"
        cells.append(row)
    ...
    assert 108 not in indices
    assert 107 in indices


def test_empty_next_checkpoint_id_excluded_even_without_route_path(
    tmp_path: Path,
) -> None:
    cells = [_write_cell(tmp_path, 18), _write_cell(tmp_path, 108)]
...
def test_terminal_yawn_moon_capture_proposes_cp108(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Last route leg (yawn_moon_210) must still install cp108."""
    monkeypatch.setenv("RE1_YAWN_RAILS_SYNC", "1")
    bridge = MagicMock()
    bridge.save_savestate.side_effect = (
        lambda path: Path(path).write_bytes(b"state")

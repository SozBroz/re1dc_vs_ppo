...
def test_dual_gamma_softlock_uses_long_horizon():
    ...
    rewards = np.array([[0.1], [0.1], [-0.7]], dtype=np.float32)  # includes softlock
    softlock = np.array([[0.0], [0.0], [-0.8]], dtype=np.float32)
    dones = np.array([[False], [False], [True]], dtype=np.bool_)
    values = np.zeros_like(rewards)
    last_values = np.array([0.0], dtype=np.float32)
    ...
    # Softlock contribution at t=0: (-0.8) * 0.998^2
    soft_at_0 = -0.8 * (0.998**2)
    # Main rewards: 0.1, 0.1, 0.1  (total - softlock = -0.7 - (-0.8) = 0.1)
    main_at_0 = 0.1 + 0.99 * (0.1 + 0.99 * 0.1)
    assert returns[0, 0] == pytest.approx(main_at_0 + soft_at_0)
    assert returns[2, 0] == pytest.approx(0.1 + (-0.8))

# Launch WH2 worker in sshuser's interactive desktop.
param(
    [string]$RepoRoot = 'C:\Users\sshuser\re1_rl',
    [string]$TaskName = 'RE1_worker_wh2'
)
$ErrorActionPreference = 'Continue'
Set-Location $RepoRoot
New-Item -ItemType Directory -Force -Path 'data\logs' | Out-Null

Write-Host 'Stopping stale EmuHawk / distributed_train / old learner task...'
Unregister-ScheduledTask -TaskName 'RE1_learner_25' -Confirm:$false -ErrorAction SilentlyContinue
...
$cmd = Join-Path $RepoRoot 'fleet\local\run_distributed_worker_workhorse2.cmd'

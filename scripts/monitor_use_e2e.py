"""Single-agent USE/menu debugger — log RAM + frames when spray USE loops."""

from __future__ import annotations

import argparse
import json
import subprocess
import sys
import time
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from re1_rl.action_mask import SELECT_SLOT_BASE, USE_ACTION
from re1_rl.bizhawk_bridge import BizHawkClient
from re1_rl.env import ACTION_NAMES, RE1Env
from re1_rl.game_session import outside_gameplay_reason
from re1_rl.memory_map import (
    EQUIPPED_SLOT_INDEX_1BASED,
    EQUIPPED_WEAPON_ID,
    GAME_MODE,
    GAME_STATE,
    MESSAGE_FLAG,
    PLAYER_HP,
    SCENE_FLAG,
)

ROOT = Path(__file__).resolve().parents[1]
EMU = ROOT / "tools" / "BizHawk-2.11.1" / "EmuHawk.exe"
ROM = ROOT / "roms" / "Resident Evil - Director's Cut.cue"
LUA = ROOT / "lua" / "re1_client.lua"
CURRICULUM = ROOT / "curriculum" / "m0_dining_to_main_hall.json"
LOG_PATH = ROOT / "data" / "use_e2e_monitor.jsonl"


def _ram_snapshot(bridge: BizHawkClient, *, episode_start_hp: int) -> dict:
    from re1_rl.item_box import read_inventory
    from re1_rl.weapon_equip import read_equipped_slot_0based

    ram = bridge.read_ram(
        [
            ("game_mode", GAME_MODE, "u8"),
            ("game_state", GAME_STATE, "u32"),
            ("msg_flag", MESSAGE_FLAG, "u8"),
            ("scene_flag", SCENE_FLAG, "u8"),
            ("player_hp", PLAYER_HP, "u16"),
            ("equipped_weapon_id", EQUIPPED_WEAPON_ID, "u8"),
            ("equipped_slot_1based", EQUIPPED_SLOT_INDEX_1BASED, "u8"),
        ]
    )
    inv = read_inventory(bridge)
    mode = int(ram.get("game_mode", 0))
    gs = int(ram.get("game_state", 0))
    outside = outside_gameplay_reason(ram, episode_start_hp=episode_start_hp)
    eq_slot = read_equipped_slot_0based(bridge)
    return {
        "game_mode": f"0x{mode:02X}",
        "game_state": f"0x{gs:08X}",
        "msg_flag": int(ram.get("msg_flag", 0)),
        "scene_flag": int(ram.get("scene_flag", 0)),
        "hp": int(ram.get("player_hp", 0)),
        "equipped_weapon_id": int(ram.get("equipped_weapon_id", 0)),
        "equipped_slot_0b": eq_slot,
        "inventory": [(int(i), int(q)) for i, q in inv],
        "in_control": bool(mode & 0x80),
        "outside_gameplay": outside,
    }


def _mask_summary(env: RE1Env) -> dict:
    from re1_rl.action_mask import USE_ACTION as UA

    m = env.unwrapped.action_masks()
    names = ACTION_NAMES
    legal = [names[i] for i in range(len(m)) if m[i]]
    return {
        "use": bool(m[UA]),
        "select_slots": [i for i in range(8) if m[SELECT_SLOT_BASE + i]],
        "legal_count": int(m.sum()),
        "legal_sample": legal[:14],
    }


def _log(line: dict) -> None:
    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with LOG_PATH.open("a", encoding="utf-8") as f:
        f.write(json.dumps(line, default=str) + "\n")
    tags = []
    if line.get("magic_report"):
        tags.append(f"magic={line['magic_report'].get('reason')}")
    if line.get("use_phase") is not None:
        tags.append(f"use_ph={line['use_phase']}")
    frames = line.get("step_emulated_frames")
    if frames is not None:
        tags.append(f"emu_frames={frames}")
    ram = line.get("ram", {})
    act = line.get("action_name") or line.get("event", "?")
    print(
        f"[use-e2e] step={line.get('step_i', '-'):>3} "
        f"act={act:<16} "
        f"gm={ram.get('game_mode')} gs={ram.get('game_state')} "
        f"ctrl={ram.get('in_control')} hp={ram.get('hp')} "
        f"outside={ram.get('outside_gameplay')!r} "
        f"{' '.join(tags)}",
        flush=True,
    )


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--port", type=int, default=7777)
    ap.add_argument("--spray-slot", type=int, default=2)
    ap.add_argument("--repeat", type=int, default=8, help="USE->slot torture loops")
    ap.add_argument("--masked-steps", type=int, default=0)
    args = ap.parse_args()
    port = int(args.port)
    spray_slot = int(args.spray_slot)

    if LOG_PATH.exists():
        LOG_PATH.unlink()

    bridge = BizHawkClient(port=port, timeout=300.0, connect_timeout=120.0)
    bridge.start_server()
    print(f"[use-e2e] port {port} — ONE EmuHawk (not fleet)", flush=True)
    proc = subprocess.Popen(
        [
            str(EMU),
            str(ROM),
            f"--lua={LUA}",
            "--socket_ip=127.0.0.1",
            f"--socket_port={port}",
        ],
        cwd=str(EMU.parent),
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
    step_i = 0
    total_emu_frames = 0
    try:
        bridge.wait_for_client()
        bridge.set_speed(100)
        env = RE1Env(
            curriculum_path=CURRICULUM,
            bridge=bridge,
            project_root=ROOT,
            async_cutscene_skip=False,
        )
        obs, info = env.reset()
        ep_hp = int(getattr(env.unwrapped, "_episode_start_hp", 96) or 96)
        ram0 = _ram_snapshot(bridge, episode_start_hp=ep_hp)
        _log({
            "event": "reset",
            "step_i": step_i,
            "ram": ram0,
            "mask": _mask_summary(env),
        })
        print(f"[use-e2e] reset mask={_mask_summary(env)} inv={ram0['inventory']}", flush=True)

        def do_step(action: int, label: str = "") -> None:
            nonlocal step_i, total_emu_frames
            step_i += 1
            pre = _ram_snapshot(bridge, episode_start_hp=ep_hp)
            pre_mask = _mask_summary(env)
            obs, rew, term, trunc, info = env.step(action)
            post = _ram_snapshot(bridge, episode_start_hp=ep_hp)
            emu_f = int(info.get("state", {}).get("step_emulated_frames", 0))
            total_emu_frames += emu_f
            _log({
                "event": label or "step",
                "step_i": step_i,
                "action": action,
                "action_name": ACTION_NAMES[action],
                "reward": float(rew),
                "ram_pre": pre,
                "ram": post,
                "mask_pre": pre_mask,
                "mask_post": _mask_summary(env),
                "magic_report": info.get("magic_report"),
                "use_phase": info.get("use_phase"),
                "step_emulated_frames": emu_f,
                "total_emu_frames": total_emu_frames,
                "terminated": term,
                "truncated": trunc,
            })
            if term or trunc:
                print(f"[use-e2e] episode ended term={term} trunc={trunc}", flush=True)

        for n in range(int(args.repeat)):
            do_step(USE_ACTION, f"use_open_{n}")
            do_step(SELECT_SLOT_BASE + spray_slot, f"use_select_spray_{n}")

        if int(args.masked_steps) > 0:
            import numpy as np

            rng = np.random.default_rng(1)
            for _ in range(int(args.masked_steps)):
                m = env.unwrapped.action_masks()
                legal = np.flatnonzero(m)
                if len(legal) == 0:
                    ram = _ram_snapshot(bridge, episode_start_hp=ep_hp)
                    print(f"[use-e2e] no legal actions {ram}", flush=True)
                    break
                do_step(int(rng.choice(legal)), "random_masked")

        print(f"[use-e2e] total_emu_frames={total_emu_frames} log={LOG_PATH}", flush=True)
        final = _ram_snapshot(bridge, episode_start_hp=ep_hp)
        print(f"[use-e2e] final {json.dumps(final)}", flush=True)
        time.sleep(0.3)
    finally:
        try:
            bridge.quit()
        except (OSError, ConnectionError):
            pass
        proc.terminate()
        try:
            proc.wait(timeout=5)
        except subprocess.TimeoutExpired:
            proc.kill()
    return 0


if __name__ == "__main__":
    raise SystemExit(main())

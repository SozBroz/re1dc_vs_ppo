"""Expand PPO action head 10 -> 11 (knife_swing) without disturbing old logits.

Preserves all trained weights; new action row starts at zero so the policy
matches the old checkpoint on actions 0-9 when obs is unchanged.

Usage:
    python scripts/transplant_knife_action.py
    python scripts/transplant_knife_action.py --src data/ppo_re1_final.zip \\
        --out data/ppo_re1_knife11
"""
from __future__ import annotations

import argparse
import sys
from pathlib import Path

import numpy as np
import torch

PROJECT_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(PROJECT_ROOT))

OLD_N_ACTIONS = 10


def build_env(n_actions: int):
    import gymnasium as gym
    from gymnasium import spaces

    from re1_rl.obs_encoder import GOAL_DIM, PROPRIO_DIM
    from re1_rl.spatial_encoder import SPATIAL_DIM, VISITED_SHAPE

    class StubRE1Env(gym.Env):
        observation_space = spaces.Dict(
            {
                "frame": spaces.Box(0, 255, shape=(84, 84, 4), dtype=np.uint8),
                "proprio": spaces.Box(-1.0, 1.0, shape=(PROPRIO_DIM,), dtype=np.float32),
                "goal": spaces.Box(-2.0, 2.0, shape=(GOAL_DIM,), dtype=np.float32),
                "spatial": spaces.Box(-2.0, 2.0, shape=(SPATIAL_DIM,), dtype=np.float32),
                "visited": spaces.Box(0.0, 1.0, shape=VISITED_SHAPE, dtype=np.float32),
            }
        )
        action_space = spaces.Discrete(n_actions)

        def reset(self, *, seed=None, options=None):
            super().reset(seed=seed)
            return self.observation_space.sample(), {}

        def step(self, action):
            return self.observation_space.sample(), 0.0, False, False, {}

    return StubRE1Env()


@torch.no_grad()
def expand_action_head(old_policy, new_policy) -> None:
    old_sd = old_policy.state_dict()
    new_sd = new_policy.state_dict()

    for k, old_t in old_sd.items():
        if k in new_sd and old_t.shape == new_sd[k].shape:
            new_sd[k].copy_(old_t)

    n_old = OLD_N_ACTIONS
    ow = old_sd["action_net.weight"]
    ob = old_sd["action_net.bias"]
    nw = new_sd["action_net.weight"]
    nb = new_sd["action_net.bias"]
    n_in = ow.shape[1]
    nw[:n_old, :n_in].copy_(ow)
    nw[n_old:, :].zero_()
    nb[:n_old].copy_(ob)
    nb[n_old:].zero_()

    new_policy.load_state_dict(new_sd)


@torch.no_grad()
def verify(old_model, new_model, n_batches: int = 8, batch: int = 16) -> float:
    env = build_env(OLD_N_ACTIONS)
    worst = 0.0
    for _ in range(n_batches):
        obs = {
            k: np.stack([env.observation_space[k].sample() for _ in range(batch)])
            for k in env.observation_space.spaces
        }
        obs["frame"] = obs["frame"].transpose(0, 3, 1, 2)
        for model in (old_model, new_model):
            model.policy.set_training_mode(False)
        a_old, _ = old_model.predict(obs, deterministic=True)
        a_new, _ = new_model.predict(obs, deterministic=True)
        if not np.array_equal(a_old, a_new):
            worst = max(worst, 1.0)
        from stable_baselines3.common.utils import obs_as_tensor

        t = obs_as_tensor(obs, old_model.device)
        v_old = old_model.policy.predict_values(t)
        v_new = new_model.policy.predict_values(t)
        worst = max(worst, (v_old - v_new).abs().max().item())
    return worst


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--src", default=str(PROJECT_ROOT / "data" / "ppo_re1_final.zip"))
    ap.add_argument("--out", default=str(PROJECT_ROOT / "data" / "ppo_re1_knife11"))
    ap.add_argument("--backup-src", action="store_true")
    args = ap.parse_args()

    from stable_baselines3 import PPO

    from re1_rl.env import ACTION_NAMES
    from re1_rl.policy_config import POLICY_KWARGS

    if len(ACTION_NAMES) != OLD_N_ACTIONS + 1:
        print(
            f"[transplant] expected {OLD_N_ACTIONS + 1} actions for expand, "
            f"got {len(ACTION_NAMES)} — use transplant_remove_aim_fire.py to narrow",
            flush=True,
        )
        return 1

    src = Path(args.src)
    if not src.is_file():
        print(f"[transplant] missing {src}", flush=True)
        return 1

    print(f"[transplant] loading {src}", flush=True)
    old_model = PPO.load(str(src), env=build_env(OLD_N_ACTIONS), device="cpu")
    print(
        f"[transplant] old steps={old_model.num_timesteps:,} "
        f"actions={OLD_N_ACTIONS}",
        flush=True,
    )

    new_model = PPO(
        "MultiInputPolicy",
        build_env(len(ACTION_NAMES)),
        policy_kwargs=POLICY_KWARGS,
        n_steps=256,
        batch_size=512,
        n_epochs=4,
        learning_rate=3e-4,
        gamma=0.99,
        ent_coef=0.01,
        device="cpu",
    )
    expand_action_head(old_model.policy, new_model.policy)
    new_model.num_timesteps = old_model.num_timesteps

    worst = verify(old_model, new_model)
    print(f"[transplant] max value drift / action mismatch: {worst:.3e}", flush=True)
    if worst > 1e-4:
        print("[transplant] FAIL: expanded net does not reproduce old policy", flush=True)
        return 1

    if args.backup_src:
        import shutil

        bak = src.with_name(src.stem + "_pre_knife11.zip")
        shutil.copy2(src, bak)
        print(f"[transplant] backed up src -> {bak}", flush=True)

    new_model.save(args.out)
    print(f"[transplant] saved {args.out}.zip", flush=True)
    print("TRANSPLANT_OK", flush=True)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
